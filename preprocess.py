# 用 RougeScorer，打分并创建 beam的candidate 需要先生成beam candidate数据
import json
...
def collect_diverse_beam_data(args):
    ...
    cands_untok = []
    cnt = 0
    with open(os.path.join(src_dir, f"{split}.source.tokenized")) as src, open(os.path.join(src_dir, f"{split}.target.tokenized")) as tgt, open(os.path.join(src_dir, f"{split}.source")) as src_untok, open(os.path.join(src_dir, f"{split}.target")) as tgt_untok: # 4个文件 src target token untoken
        with open(os.path.join(src_dir, f"{split}.out.tokenized")) as f_1, open(os.path.join(src_dir, f"{split}.out")) as f_2: # F_1 tokenized out. F_2 out
